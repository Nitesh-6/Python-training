...
---------------------------------------
State    : Attributes/Fields : eid, name, sal, mobile, address, emailid
...
Entity   : Class 
Variables: Attributes/Fields  : State
...
'''
Disadvantages:
    1. State: No Security for input data 
    2. Behavior: No grouping of functions 
    3. Difficult to deal with large volumes of data(State)
    4. Function Definition: Parameters definition is difficult
    5. Code Pollution
'''
...
class Employee:
    # State
    def __init__(self, eid, name, sal):  # init method is a Constructor
        self.eid = eid  # self.eid = 100
        self.name = name
        self.sal = sal

    # Behavior
    def get_einfo(self):  # Method : instance method
        print("Employee Data: ", self.eid, self.name, self.sal)


# 2. Object creation
madhu = Employee(100, "Madhu Nettem", 10000)

# 3. Method Call
madhu.get_einfo()
Employee.get_einfo(madhu)  # avoid using this way
...
'''
At runtime, Object creation
            # 1. Divides RHS into 2 parts 
                      I. Employee  
#                    II. (100, 'Vikram Magala', 15000)
              2. Creates wrapper with empty memory slot
...
# Step2: Create Object
bhargav = Student(100, "Bhargav Sai M ", 0)
# Step3: Method Call
bhargav.get_sinfo()
